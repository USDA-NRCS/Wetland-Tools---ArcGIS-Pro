...
## **Version 2.1.0 (07/19/2024; Production Release):**

### **New Features/Changes**
- Renamed 2.7 Pro template files (.aprx) to 2.9
- Deleted empty GDB and TBX files that were autogenerated during template creation, as they are not used by the tool
- Updated versions of bundled and embedded python packages in the tool folders
- Removed System Settings parameters from the Create Project tool
- Added feature for Create Project tool to zoom to the extent of the Site CLU layer when it finishes running
- Updated tool to handle apostrophes in USDA office names in pick lists and processing
- Updated census lookup table with corrections for a few locations
- Revised Extract CLU subscript using the ArcGIS for Python API
- Updated all references of gis-testing to gis-states
- Added FSA FGDC metadata to the Site CLU Master and Site CLU feature classes
- Moved NWI download out of the Create Project tool and into a standalone, optional tool in toolset B.
- Removed Portal token and connection validation checks from the Download NWI tool
- Switched source URL in NWI functions to use the NRCS map service copy, rather than the NRCS feature service
- Fixed typo in Download NWI Data tool dialogue
- Made contour interval a required parameter in Create Elevation Derivatives
- Made create depth grid an optional parameter in Create Elevation Derivatives
- Added a pick-list to select an NRCS bare earth elevation service, rather than prompting for a service URL or layer file when using NRCS Image Service otpion in Create Elevation Derivatives
- Added a third option for input data to allow for a non-NRCS image service, which requires using a layer file of the service which must be provided by State GIS Specialists in their deployments to users
- Updated PLSS service URLs to NRCS hosted service instances to remove reliance on an external agency for service uptime
- Fixed typo in Export Determination Map tool dialogue
- Increased the length of CLU number list in a temporary processing layer from 512 to 1000 to handle tracts with extremely high numbers of CLU fields
...
States that have adopted version 2.0.0 of the tools already can download a later version, and only need to update the following:
